...
static void aft_nr(int iter, double eps, int printlevel,
    ...
    *loglik = aft_fun(bdim, beta, vex);
    gaft_fun(bdim, beta, dloglik, vex);
    g2aft_fun(bdim, beta, variance, vex);
    if (printlevel){
      Rprintf("[aft_nr] bdim is: %d\n", bdim);
      Rprintf("[aft_nr] dloglik is\n");
      for (i = 0; i < bdim; i++){
	Rprintf("%f ", dloglik[i]);
      }
      Rprintf("\n");
...
/*
    ...
    xin[*bdim - 2] = 0.0;
    Fmin = aft_fun(*bdim, xin, vex);
    gaft_fun(*bdim, xin, dloglik, vex);
    g2aft_fun(*bdim, xin, variance, vex);
    Rprintf("\n!!b = 0.1:\n");
    Rprintf("f = %f\n", Fmin);
    Rprintf("fp: ");
    for (i = 0; i < *bdim; i++){
...
/*
    trace = 0; 
    nmmin(*bdim, xin, beta, &Fmin, aft_fun, 
	  fail, *eps, *eps, vex, 
	  1.0, 0.5, 2.0, trace,
	  &fncount, maxiter);
    trace = *printlevel;

    Fmin = aft_fun(*bdim, beta, vex);
    gaft_fun(*bdim, beta, dloglik, vex);
    loglik[1] = -Fmin;
    if (trace){
	Rprintf("\nEfter 'nmmin' [aftreg]: loglik = %f\n", -Fmin);
...
/* 'Mask out' the regression coefficients: */
    ...
    Fmin = aft_fun(*bdim, beta, vex);
    if (trace)
      Rprintf("before vmmin [aftsup]: loglik = %f\n", Fmin);

/* Estimate the 'null' model (only scale and shape): */
    vmmin(*bdim, beta, &Fmin,
	  aft_fun, gaft_fun, maxiter, trace,  
...
/* Estimate the full model: */
    ...
    gaft_fun(*bdim, beta, dloglik, vex);
    if (trace){
	Rprintf("\n[aftreg] After vmmin; score is\n");
	for (i = 0; i < *bdim; i++){
	    Rprintf("%f ", dloglik[i]);
	}
	Rprintf("\n\n");
	Rprintf("\n[aftreg] After vmmin; beta is\n");
	for (i = 0; i < *bdim; i++){
	    Rprintf("%f ", beta[i]);
	}
	Rprintf("\n\n");
...
/*
	...
	*fail = info;
	Rprintf("Inverse failed [aftreg]; info = %d\n", info);
	return;
    } 
*/
/* End testing */
...
/* Take some Newton-Raphson steps: */
	...
	Rprintf("Variance (in [aftreg]) after N-R:\n");
	for (i = 0; i < *bdim; i++){
	    for (j = 0; j < *bdim; j++){
		Rprintf("%f ", variance[i + j * (*bdim)]);
	    }
