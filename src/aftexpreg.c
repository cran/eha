...
static void gaftexp_fun(int n, double *beta, double *dloglik, void *vex){
/* Here shape is FIXED, i.e. NOT estimated, and NOT part of beta! */
    int i, j;
    double alpha, gamma;
    double *fp;
	...
	alpha = beta[mb + i];
	gamma = log(ex->pfix[i]);
	d_loglik_aft(&dist, &mb, beta, &alpha, &gamma,
		    &nn, ex->z + *(ex->mb) * start,
		    ex->time0 + start, ex->time + start,
...
static void g2aftexp_fun(int n, double *beta, double *d2loglik, void *vex){
/* Here shape is FIXED, i.e. NOT estimated, and NOT part of beta! */
/* n is the dimension of beta, i.e., mb + ns. */
	...
	alpha = beta[mb + i];
	gamma = log(ex->pfix[i]);
	d2_loglik_aft(&dist, &mb, beta, &alpha, &gamma,
		     &nn, ex->z + *(ex->mb) * start,
		     ex->time0 + start, ex->time + start,
...
static void aftexp_nr(int iter, double eps, int printlevel,
    ...
    *loglik = aftexp_fun(bdim, beta, vex);
    gaftexp_fun(bdim, beta, dloglik, vex);
    g2aftexp_fun(bdim, beta, variance, vex);
...
/* Done! The afterwork: */
	...
	*fail = info;
	...
	Rprintf("*** Iteration %d\n", iter);
	if (*conver){
	    Rprintf("[nr_aft] Convergence!\n");
	}else{
	    Rprintf("[nr_aft] NO Convergence!\n");
	}
	Rprintf("loglik = %f\n", *loglik);
    }
...
void aftexpsup(int *iter, double *eps, int *printlevel,
...
/*
    ...
    xin[*bdim - 2] = 0.0;
    Fmin = aftexp_fun(*bdim, xin, vex);
    gaftexp_fun(*bdim, xin, dloglik, vex);
    g2aftexp_fun(*bdim, xin, variance, vex);
    Rprintf("\n!!b = 0.1:\n");
    Rprintf("f = %f\n", Fmin);
    Rprintf("fp: ");
    for (i = 0; i < *bdim; i++){
...
/*
    trace = 0; 
    nmmin(*bdim, xin, beta, &Fmin, aftexp_fun, 
	  fail, *eps, *eps, vex, 
	  1.0, 0.5, 2.0, trace,
	  &fncount, maxiter);
    trace = *printlevel;

    Fmin = aftexp_fun(*bdim, beta, vex);
    gaftexp_fun(*bdim, beta, dloglik, vex);
    loglik[1] = -Fmin;
    if (trace){
	Rprintf("\nEfter 'nmmin' [aftexpreg]: loglik = %f\n", -Fmin);
...
/* Estimate the 'null' model (only scale and shape): */
    vmmin(*bdim, beta, &Fmin,
	  aftexp_fun, gaftexp_fun, maxiter, trace,  
...
/* Estimate the full model: */
    ...
    gaftexp_fun(*bdim, beta, dloglik, vex);
    if (trace){
	Rprintf("\n[aftexpreg] After vmmin; score is\n");
	for (i = 0; i < *bdim; i++){
	    Rprintf("%f ", dloglik[i]);
	}
	Rprintf("\n\n");
	
	Rprintf("\n[aftexpreg] After vmmin; beta is\n");
	for (i = 0; i < *bdim; i++){
	    Rprintf("%f ", beta[i]);
	}
	Rprintf("\n\n");
...
/* Testing... */
	...
	*fail = info;
	Rprintf("Inverse failed [aftexpreg]; info = %d\n", info);
	return;
    } 
/* End testing */
...
/* Take some Newton-Raphson steps: */
    ...
    aftexp_nr(*iter, *eps, *printlevel, 
	  *bdim, beta, 
	  (loglik + 1), dloglik, variance, 
	  conver, fail, ex);

    if (trace){
	Rprintf("Variance (in [aftexpreg]) after N-R:\n");
	for (i = 0; i < *bdim; i++){
	    for (j = 0; j < *bdim; j++){
		Rprintf("%f ", variance[i + j * (*bdim)]);
	    }
