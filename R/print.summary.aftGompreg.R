#' Prints summary.aftGompreg objects
#' 
...
#' @keywords survival
#' @export
print.summary.aftGompreg <- function(x, 
                                  digits = max(getOption("digits") - 3, 3),
                                  short = FALSE, ...){
    if (!("summary.aftGompreg" %in% class(x))){
        stop("Only for 'summary.aftGompreg' ojects.")
    }
    ...
    if (!is.null(x$fail)) {
        if (x$fail != 0){
            cat(" print.summary.aftGompreg failed with: ")
            stop(x$fail)
        }
    }
